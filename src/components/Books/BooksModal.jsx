...
const BooksModal = ({ open, setOpen, fetchBooks, type, setType }) => {
  ...
  const onSubmit = async (data) => {
    ...
    const res =
      "POST" +
      `/books` +
      JSON.stringify({ isbn: data.isbn.toString() }) +
      dataa.secret;
    ...
    setLoading(true);
    await axios
      .post("https://0001.uz/books", JSON.stringify(d), {
        headers: {
      ...
      .catch((e) => {
        setMessage(e.response.data.message), setAlert(true);
      });
    setLoading(false);
  };
